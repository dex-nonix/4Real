...
class NxDaemonManager:
    ...
    def __init__(self):
        ...
        self.daemons: Dict[str, NxBaseDaemon] = {}
        self.emitter = pyee_asyncio.AsyncIOEventEmitter()

    def add_daemon(self, daemon: NxBaseDaemon):
        """
        ...
        try:
            ...
            self.emitter.emit(NxDaemonManagerEvents.DAEMON_ADDED.value, daemon)
        except Exception as e:
            self.logger.error(f"Error adding daemon '{daemon.name}': {e}")
            self.emitter.emit(NxDaemonManagerEvents.MANAGER_ERROR.value, self, e)
    ...
    def remove_daemon(self, name: str):
        ...
        try:
            if name in self.daemons:
                ...
                self.emitter.emit(NxDaemonManagerEvents.DAEMON_REMOVED.value, daemon)
        except Exception as e:
            self.logger.error(f"Error removing daemon '{name}': {e}")
            self.emitter.emit(NxDaemonManagerEvents.MANAGER_ERROR.value, self, e)

    def _register_daemon_events(self, daemon: NxBaseDaemon):
        """
    ...
    async def start_all(self):
        ...
        """
        try:
            self.emitter.emit(NxDaemonManagerEvents.BEFORE_START_ALL.value, self)
            for daemon in self.daemons.values():
                await daemon.start()
            self.emitter.emit(NxDaemonManagerEvents.AFTER_START_ALL.value, self)
        except Exception as e:
    ...
    async def stop_all(self):
        ...
        """
        try:
            self.emitter.emit(NxDaemonManagerEvents.BEFORE_STOP_ALL.value, self)
            for daemon in self.daemons.values():
                await daemon.stop()
            self.emitter.emit(NxDaemonManagerEvents.AFTER_STOP_ALL.value, self)
        except Exception as e:
