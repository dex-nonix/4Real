...
class BaseCrudService(ABC, Generic[ModelType]):
    """
    Abstract base service for CRUD operations.
    Internal services should inherit from this class and define their CRUDConfig.
    """
    config: CRUDConfig
    ...
    async def get_one(self, item_id: int) -> ModelType:
    ...
    async def bulk_update(self, ids: List[int], data: dict):
    ...
    async def _validate_unique_fields(self, data: BaseModel, item_id: Optional[int], session):
    ...
    async def _get_count(self, session, query_params: dict) -> int:
        query = select(self.model)
    ...
    async def selector(self, q: Optional[str]) -> List[Dict]:
        """Get selector data for dropdowns/autocomplete"""
        async with AsyncSessionLocal() as session:
            selector_cfg = self.config.selector
            query = select(self.model)
