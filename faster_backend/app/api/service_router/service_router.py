...
class ServiceRouter:
    ...
    async def get_service_info(self, service_name: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            'websocket_channels': [info for info in self._websocket_channels.values() if info['service_name'] == service_name]
        }
    ...
    def _add_utility_endpoints(self) -> None:
        ...
        async def get_compact_overview(services: str = None):
            ...
            try:
                ...
                yaml_overview = await self.compact_generator._convert_openapi_to_compact_yaml(openapi_spec, service_filter)
    ...
    async def _process_websocket_message(self, channel: str, message: str, websocket):
        ...
        try:
            ...
            if data.get('type') == 'ping':
                await websocket.send_text(json.dumps({'type': 'pong', 'timestamp': __import__('datetime').datetime.now().isoformat()}))
