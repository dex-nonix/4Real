...
async def register_all_services(service_router: ServiceRouter, app: FastAPI) -> None:
    ...
    await service_router.register_service("artists", ArtistService)
    await service_router.register_service("albums", AlbumService)
    await service_router.register_service("tracks", TrackService)
    await service_router.register_service("personas", PersonaService)
    await service_router.register_service("ai-analysis-results", AIAnalysisResultService)
    await service_router.register_service("ai-model-mappings", AIModelMappingService)
    await service_router.register_service("ai-providers", AIProviderService)
    await service_router.register_service("chat-histories", ChatHistoryService)
    await service_router.register_service("chat-messages", ChatMessageService)
    await service_router.register_service("chat-sessions", ChatSessionService)
    await service_router.register_service("file-categories", FileCategoryService)
    await service_router.register_service("file-links", FileLinkService)
    await service_router.register_service("internal-tools", InternalToolService)
    await service_router.register_service("mcp-servers", MCPServerService)
    await service_router.register_service("persona-mcp-servers", PersonaMCPServerService)
    await service_router.register_service("persona-tool-access", PersonaToolAccessService)
    await service_router.register_service("rhyme-techniques", RhymeTechniqueService)
    await service_router.register_service("styles", StyleService)
    await service_router.register_service("tool-invocation-logs", ToolInvocationLogService)

    # Register special services (with app parameter for ChatService)
    await service_router.register_service("files", FileService)
    await service_router.register_service("chat", ChatService, app)
